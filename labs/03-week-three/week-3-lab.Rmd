...
suppressMessages({
  install.packages(c("tidymodels", "gghighlight", "glue", "ggmosaic", "ggridges", "gridExtra", "infer", "janitor", "knitr", "kableExtra", "maps", "openintro", "patchwork", "quantreg", "tidyverse", "scales", "skimr", "caret", "palmerpenguins", "survival", "waffle", "ggrepel", "ggpubr", "openintro"))
})
```
...
rootdir <- ("~/Documents/MEDS_2021_2022/02_Fall_21/EDS222_stats_Carleton/EDS-222-stats")
...
plot1 <- ggplot(data = possum, aes(x = tail_l, y = total_l)) +
  geom_point() +
  labs(x = "tail length (cm)",
       y = "total length (cm)",
       title = "Possume Measurements")


plot2 <- ggplot(data = possum, aes(x = tail_l, y = head_l)) +
  geom_point() +
  labs(x = "tail length (cm)",
       y = "head length (cm)",
 ...
 **Tail length as a function of total length appears to be positively correlated. Tail length as a function of head length does not appear to be correlated. **
...
# (i) finds the correlation between total_l & tail_l
tot_tail_cor <- possum %>% 
  summarize(total_tail = cor(total_l, tail_l)) %>% 
  gt()


head_tail_cor <- possum %>% 
  summarize(head_v_tail = cor(head_l, tail_l)) %>% 
  gt()
...
beta <- summary(lm(total_l ~ tail_l, data = possum))
...
**Would be concerned when values are below 30cm because all possums have tails **
...
ggplot(data = possum, aes(y = total_l, x = tail_l)) +
  ...
  # key function for visualizing linear regression, model = linear model, relate 
...
errors <- possum %>% 
  ...
  mutate(model = total_l - pred)

# histogram
errors_hist <- ggplot(data = errors, aes(x = model)) +
  geom_histogram()
errors_hist

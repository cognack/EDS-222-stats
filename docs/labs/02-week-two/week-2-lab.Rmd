...
# rootdir <- (C:/Users/stvec/Documents/MEDS_2021_2022/02_Fall_21/EDS222_stats_for_env_data_sci_Carleton/R)
...
# source(file.path(rootdir,"labs","_common-local.R")
...
loans <- loans_full_schema %>% 
  mutate(
    ...
    homeownership = fct_relevel(homeownership, "rent"),
    application_type = fct_relevel(application_type, "joint")
...
# create bar plot using 'open intro" color coding

p_count <- ggplot(data = loans, aes(x = homeownership)) + 
  geom_bar(fill = IMSCOL["pink", "full"]) +
  labs(x = "Homeownership", y = "Count")



# bar plot showing proportion of loans in each homeownership
p_proportion <- loans %>% 
  # count creates a new tibble with a total counts with column header 'n'
  count(homeownership) %>% 
  mutate(proportion = n / sum(n)) %>% 
  ggplot(aes(x = homeownership, y = proportion)) +
  geom_col(fill = IMSCOL["black", "full"]) +
...
p_stacked / (p_count + p_proportion)
...
p_standardized <- ggplot(loans, aes(x = homeownership, fill = application_type)) +
  ...
  # position = "fill" <- creates the full position which adds up to 1
